...
import org.example.ecommerce.dto.InventorySalesDTO;
...
public class ProductService {
    ...
    public void save(Product product, MultipartFile[] images, MultipartFile[] inventoryImages, Integer shopId) {
        ...
        if (product.getInventories() != null) {
            ...
            for (int i = 0; i < product.getInventories().size(); i++) {
                ...
                if (inventory.getColor() == null) inventory.setColor("Mặc định");
                if (inventory.getDimension() == null) inventory.setDimension("Mặc định");
    ...
    public List<ProductSalesDTO> getProductSalesData(Integer shopId, String status) {
        ...
        return products.stream().map(this::mapToProductSalesDTO).collect(Collectors.toList());
    }
